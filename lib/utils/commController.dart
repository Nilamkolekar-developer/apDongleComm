...
class CommController extends GetxController {
    ...
    return completer.future
        .timeout(
          ...
          onTimeout: () {
            subscription?.cancel();
            String errorMsg =
                "Timeout: Got ${accumulated.length} of $length bytes";
            print("⚠️ $errorMsg");
  ...
  Future<Uint8List> getWifiResponse() async {
    if (connectivity.value == Connectivity.wiFi) {
      try {
        print(
          "WiFi Communication : ---------INSIDE READ DATA -----------${DateTime.now()}",
        );
        final Uint8List trgtlen = await _readExactBytes(2);
        print(
          "WiFi Communication : ---------Target Length Response Received = ${_byteArrayToHex(trgtlen)} -----------",
        );
        ...
        print(
          "WiFi Communication : ---------Response Received = ${_byteArrayToHex(retArray)} -----------",
        );
        return retArray;
      } catch (e) {
      ...
      try {
        ...
        var (bool readAgain, Uint8List? decodedResp) = decodeRP1210Message(
          rawResp,
        );
      ...
      // --- BRANCH 2: RP1210 / DOIP USB ---
        ...
        if (resp.length >= 4 &&
          ...
          if (resp.length > msgLen) {
            ...
            _showToast(
              "Fragmented packet detected, stitching...",
              isError: false,
            );
